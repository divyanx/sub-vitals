 ...
 *   2. Correct headers when token is set (x-redlattuce-signature, x-redlattuce-timestamp).
...
function mockSettings(token: string, url = 'https://studio.redlattuce.app') {
...
describe('forwardToStudio', () => {
  ...
  it('calls fetch with correct headers and body when token is set', async () => {
    ...
    expect(url).toBe('https://studio.redlattuce.app/api/webhooks/devvit');
    expect((init.headers as Record<string, string>)['content-type']).toBe('application/json');
    expect((init.headers as Record<string, string>)['x-redlattuce-timestamp']).toMatch(/^\d+$/);

    const sig = (init.headers as Record<string, string>)['x-redlattuce-signature'];
