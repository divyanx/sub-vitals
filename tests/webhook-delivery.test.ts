...
describe('deliverWebhook', () => {
  ...
  it('sets X-SubVitals-Signature header in sha256=<hex> format', async () => {
    ...
    expect(headers['x-sub-vitals-signature']).toMatch(/^sha256=[0-9a-f]{64}$/);
    expect(headers['x-sub-vitals-timestamp']).toMatch(/^\d+$/);
    expect(headers['x-sub-vitals-event']).toBe('post-tag');
  });
  ...
  it('HMAC signature is stable: same secret+payload → same hex', async () => {
    ...
    expect(sig1['x-sub-vitals-signature']).toMatch(/^sha256=[0-9a-f]{64}$/);
    expect(sig2['x-sub-vitals-signature']).toMatch(/^sha256=[0-9a-f]{64}$/);
  });
