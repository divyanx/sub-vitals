 ...
 * Tier: CORE · Phase 2
 *
 * RedLattuce module that registers with the dispatcher but does no event
...
export const studioBridgeModule: RedLatticeModule = {
  name: 'studio-bridge',
  description: 'Forwards selected events to RedLattuce Studio via authenticated outbound webhook.',
  tier: 'core',
        ...
        try {
          const res = await fetch(endpoint, {
            ...
            headers: {
              ...
              'x-redlattuce-signature': `sha256=${hexSig}`,
              'x-redlattuce-timestamp': String(ts),
            },
            body: pingBody,
            signal: ctrl.signal,
          });
