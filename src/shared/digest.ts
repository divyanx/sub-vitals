...
export function buildWeeklyDigest(stats: DigestStats, subredditName: string): string {
  ...
  return [
    `# RedLattice Weekly Digest — r/${subredditName}`,
    `**Period:** ${from} → ${to}`,
    '',
    '## Volume',
    ...
    '',
    '---',
    '_Generated by RedLattice · [Dashboard](https://reddit.com) · unsubscribe by disabling the weekly-digest scheduler task_',
  ].join('\n');
}
