...
export function buildWeeklyDigest(stats: DigestStats, subredditName: string): string {
  ...
  return [
    `# RedLettuce Weekly Digest — r/${subredditName}`,
    `**Period:** ${from} → ${to}`,
    '',
    '## Volume',
    ...
    '',
    '---',
    '_Generated by RedLettuce · [Dashboard](https://reddit.com) · unsubscribe by disabling the weekly-digest scheduler task_',
  ].join('\n');
}
