...
**Why.**
- RedLattuce is fundamentally a multi-tab analytics dashboard. Devvit Web is purpose-built for that. Blocks is purpose-built for in-feed games/widgets.
...
**Context.** RedLattuce has two surfaces:
...
(Key prefix is `rl:` for RedLattuce; the legacy spec used `bp:`. We're not maintaining backward compatibility — this is a fresh build.)
...
**Decision.** Our `RedLattuceModule` interface takes only the typed event payload, no `ctx`. Modules import what they need.
...
**Consequences.** RedLattuce positions cleanly as "the on-Reddit analytics tier"; customers wire the CSV pull into their existing CX stack in 30 minutes. No partnership negotiation needed.
