...
A mod clicks "RedLettuce · Tag issue" on a post. The flow:
...
`@devvit/redis`'s `RedisClient` exposes the standard Redis ops: `set/get/del/exists/incr/incrBy/expire/hSet/hGet/hIncrBy/hGetAll/hDel/zAdd/zRange/zRem/zRemRangeByScore/...` Per-installation isolation is automatic — two subreddits installing RedLettuce can use the same key strings without colliding.
